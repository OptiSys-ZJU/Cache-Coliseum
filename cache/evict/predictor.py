...
class CodexPredictor(ReuseDistancePredictor):
    """GPT-5.2-Codex based predictor for cache eviction.
    
    Enabled for all clients via the ENABLE_GPT52_CODEX feature flag.
    Uses advanced transformer-based prediction for cache line eviction.
    """
    
    def __init__(self, shared_model):
        """Initialize CodexPredictor with shared model.
        
        Args:
            shared_model: CodexModel instance shared across predictors
        """
        self._model = shared_model
        
    def predict_score(self, ts, pc, address, cache_state):
        """Predict single address score (not used for Codex)."""
        return None
    
    def refresh_scores(self, ts, pc, address, cache_state: Tuple[List, List]) -> List[Union[int, float, str]]:
        """Refresh all cache line scores using GPT-5.2-Codex model.
        
        Args:
            ts: Current timestamp
            pc: Program counter of current access
            address: Memory address being accessed
            cache_state: Current cache state (address, pc) pairs
            
        Returns:
            List of eviction scores for each cache line
        """
        cache_access = SimpleNamespace()
        cache_access.pc = pc
        cache_access.address = address
...
class CodexStatePredictor(HybridStatePredictor):
    """State-based predictor using GPT-5.2-Codex for cache simulation."""
    
    def __init__(self, associativity, shared_model):
        """Initialize CodexStatePredictor.
        
        Args:
            associativity: Cache associativity
            shared_model: CodexModel instance for prediction
        """
        super().__init__(associativity, CodexPredictor(shared_model))