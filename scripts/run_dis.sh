...
for dataset in "${datasets[@]}"; do
    ...
    nohup python -m benchmark --boost_fr --dataset "$dataset" --oracle --pred oracle_dis --noise_type dis --dump_file --output_root_dir stat > "logs/benchmark/oracle/${dataset}_dis.log" 2>&1 &
    pids+=($!)
done
